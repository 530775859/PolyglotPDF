...
class ServerThread(Thread):
    """后台运行的 Flask 服务器线程。"""
    def __init__(self, flask_app, host="127.0.0.1", port=5100):
        super().__init__()
        self.host = host
        self.port = port
        self.app = flask_app
...
def open_browser():
    webbrowser.open_new("http://127.0.0.1:5100")
# 修改主程序初始化部分
...
if __name__ == "__main__":
    ...
    try:
        # 创建并启动服务器
        server = ServerThread(app, host="127.0.0.1", port=5100)
        server.daemon = True  # 设置为守护线程
        server.start()
        print("服务器已在 http://127.0.0.1:5100 运行...")
